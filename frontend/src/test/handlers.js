...
export const handlers = [
  ...
  http.post(`${API}/auth/refresh-token`, () => HttpResponse.json(apiResponse({ token: "new-access-token", refreshToken: "refresh-token", user: customerUser }))),
  http.get(`${API}/users/me`, ({ request }) => {
    const auth = request.headers.get("authorization") || "";
    return HttpResponse.json(apiResponse(auth.includes("admin") ? adminUser : customerUser));
  }),
  http.get(`${API}/storefront/home`, () => HttpResponse.json(apiResponse({
    featuredBooks: [book],
  ...
  http.get(`${API}/products`, () => HttpResponse.json(apiResponse(pageResponse([book, secondBook], { pageSize: 12, totalElements: 2 })))),
  http.get(`${API}/products/:slug`, ({ params }) => HttpResponse.json(apiResponse({ ...book, slug: params.slug || book.slug }))),
  http.get(`${API}/products/:slug/reviews`, () => HttpResponse.json(apiResponse(pageResponse([{ ...review, approved: true, visible: true, adminReply: "Thanks for reading with Aivira." }])))),
  ...
  http.get(`${API}/users/me/addresses`, () => HttpResponse.json(apiResponse([
    {
      id: 801,
  ...
  http.post(`${API}/checkout`, () => HttpResponse.json(apiResponse({
  ...
  http.post(`${API}/payments/groups/:code/retry`, () => HttpResponse.json(apiResponse({ ...paymentGroup, paymentUrl: "https://pay.example.test/redirect" }))),
  http.get(`${API}/admin/dashboard/summary`, () => HttpResponse.json(apiResponse({
    revenue: 810000,
    orderCount: 3,
  ...
  http.get(`${API}/admin/dashboard/sales`, () => HttpResponse.json(apiResponse({ points: [{ date: "2026-06-11", revenue: 810000, orderCount: 1 }] }))),
  http.get(`${API}/admin/dashboard/orders`, () => HttpResponse.json(apiResponse({ statusCounts: [{ status: "PENDING_CONFIRMATION", count: 1 }] }))),
  http.get(`${API}/admin/dashboard/top-books`, () => HttpResponse.json(apiResponse({ books: [{ ...book, quantitySold: 2, revenue: 810000 }] }))),
  ...
  http.put(`${API}/admin/orders/:id/confirm`, () => HttpResponse.json(apiResponse({ ...order, orderStatus: "CONFIRMED" }))),
  http.get(`${API}/admin/coupons`, () => HttpResponse.json(apiResponse(pageResponse([{ id: 901, code: "AIVIRA10", type: "PERCENT", value: 10, active: true, usedCount: 0 }])))),
  http.get(`${API}/admin/promotions`, () => HttpResponse.json(apiResponse(pageResponse([{ id: 902, promotionName: "Architecture Week", promotionType: "FIXED", value: 50000, promotionScope: "PRODUCT", targetId: book.id, active: true }])))),
  http.get(`${API}/admin/reviews`, () => HttpResponse.json(apiResponse(pageResponse([review])))),
  http.put(`${API}/admin/reviews/:id/moderate`, async ({ request }) => HttpResponse.json(apiResponse({ ...review, ...(await request.json()) }))),
  http.put(`${API}/admin/reviews/:id/reply`, async ({ request }) => HttpResponse.json(apiResponse({ ...review, ...(await request.json()) }))),
  http.get(`${API}/admin/payments/groups/:code`, () => HttpResponse.json(apiResponse(paymentGroup))),
  http.post(`${API}/admin/payments/groups/:code/reconcile`, () => HttpResponse.json(apiResponse({
    paymentGroupCode: paymentGroup.paymentCode,
    method: "VNPAY",
