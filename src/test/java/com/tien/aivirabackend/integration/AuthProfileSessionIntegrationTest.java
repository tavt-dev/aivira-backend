...
class AuthProfileSessionIntegrationTest extends AbstractIntegrationTest {
    ...
    void registerVerifyAndLogin_shouldCreateActiveUserAndIssueTokens() throws Exception {
        ...
        MvcResult login = login("alice", PASSWORD).andExpect(status().isOk()).andExpect(cookie().exists("refreshToken"))
    ...
    void login_shouldRejectUnverifiedAccount() throws Exception {
        ...
        login("unverified", PASSWORD).andExpect(status().isUnauthorized())
    ...
    void refreshToken_shouldRotateAndDetectReuse() throws Exception {
        ...
        MvcResult refreshResult = mockMvc
                .perform(post("/auth/refresh-token").contentType(APPLICATION_JSON)
        ...
        mockMvc.perform(post("/auth/refresh-token").contentType(APPLICATION_JSON)
                .content(json(Map.of("refreshToken", firstRefreshToken)))).andExpect(status().isUnauthorized())
    ...
    void logout_shouldRevokeCurrentRefreshTokenAndClearCookie() throws Exception {
        ...
        mockMvc.perform(
                post("/auth/logout").contentType(APPLICATION_JSON).content(json(Map.of("refreshToken", refreshToken))))
    ...
    void protectedProfileEndpoints_shouldRequireTokenAndAllowCurrentUserUpdates() throws Exception {
        ...
        mockMvc.perform(get("/users/me").header("Authorization", "Bearer " + accessToken)).andExpect(status().isOk())
                .andExpect(jsonPath("$.data.username").value("profile_user"));

        mockMvc.perform(put("/users/me").header("Authorization", "Bearer " + accessToken).contentType(APPLICATION_JSON)
                .content(json(Map.of("firstName", "Updated", "lastName", "User", "gender", "MALE"))))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.firstName").value("Updated"))
    ...
    void sessions_shouldListAndRevokeOnlyCurrentUserSession() throws Exception {
        ...
        MvcResult sessions = mockMvc.perform(get("/auth/sessions").header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data[0].sessionId").isNotEmpty()).andReturn();

        String sessionId = read(sessions, "/data/0/sessionId").asText();

        mockMvc.perform(
                delete("/auth/sessions/{sessionId}", sessionId).header("Authorization", "Bearer " + accessToken))
    ...
    void logoutAllAndChangePassword_shouldInvalidateOldAccessTokenByTokenVersion() throws Exception {
        ...
        mockMvc.perform(put("/users/me/password").header("Authorization", "Bearer " + secondAccessToken)
                .contentType(APPLICATION_JSON).content(json(Map.of("currentPassword", PASSWORD, "newPassword",
                        "NewPassword123!", "confirmPassword", "NewPassword123!"))))
                .andExpect(status().isOk());
    ...
    private void register(String username, String email) throws Exception {
        mockMvc.perform(post("/auth/register").contentType(APPLICATION_JSON).content(json(Map.of("username", username,
                "password", PASSWORD, "email", email, "firstName", "Test", "lastName", "User"))))
    ...
    private void verifyUser(User user) throws Exception {
        ...
        mockMvc.perform(post("/auth/verify-user").contentType(APPLICATION_JSON)
