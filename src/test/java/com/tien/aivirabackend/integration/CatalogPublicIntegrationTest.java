...
class CatalogPublicIntegrationTest extends AbstractIntegrationTest {
    ...
    void publicCatalogEndpoints_shouldExposeOnlyVisibleActiveProductsWithoutToken() throws Exception {
        Category category = categoryRepository.save(Category.builder().categoryName("Fashion").slug("fashion")
                .description("Fashion").displayOrder(0).active(true).visible(true).build());
        ...
        mockMvc.perform(get("/products").param("keyword", "product")).andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalElements").value(1))
                .andExpect(jsonPath("$.data.data[0].slug").value("active-product"));

        mockMvc.perform(get("/products/{slug}", "active-product")).andExpect(status().isOk())
    ...
    void publicProductSearch_shouldFilterByBookMetadataAndSortByName() throws Exception {
        ...
        saveProduct("BOOK-1", "beta-book", "Beta Book", ProductStatus.ACTIVE, category, "Nguyen Nhat Anh",
                "Tre Publishing", "978-604-001");
        saveProduct("BOOK-2", "alpha-book", "Alpha Book", ProductStatus.ACTIVE, category, "Ursula Le Guin",
                "Aivira Press", "978-604-002");

        mockMvc.perform(get("/products").param("author", "nguyen")).andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalElements").value(1))
                .andExpect(jsonPath("$.data.data[0].slug").value("beta-book"));

        mockMvc.perform(get("/products").param("publisher", "aivira")).andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalElements").value(1))
                .andExpect(jsonPath("$.data.data[0].slug").value("alpha-book"));

        mockMvc.perform(get("/products").param("isbn", "002")).andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalElements").value(1))
                .andExpect(jsonPath("$.data.data[0].slug").value("alpha-book"));

        mockMvc.perform(get("/products").param("keyword", "guin")).andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalElements").value(1))
                .andExpect(jsonPath("$.data.data[0].slug").value("alpha-book"));

        mockMvc.perform(get("/products").param("sort", "name_asc")).andExpect(status().isOk())
                .andExpect(jsonPath("$.data.data[0].slug").value("alpha-book"))
    ...
    private void saveProduct(String sku, String slug, String productName, ProductStatus status, Category category,
            String bookAuthor, String publisher, String isbn) {
        Product product = Product.builder().category(category).sku(sku).productName(productName).slug(slug)
                .description("Product description").bookAuthor(bookAuthor).publisher(publisher).isbn(isbn)
                .price(BigDecimal.valueOf(100)).stockQuantity(5).soldCount(0).active(true).featured(false)
                .status(status).build();
        ProductVariation variation = ProductVariation.builder().product(product).sku(sku + "-VAR").color("Black")
                .size("M").additionalPrice(BigDecimal.ZERO).stockQuantity(5).active(true).build();
