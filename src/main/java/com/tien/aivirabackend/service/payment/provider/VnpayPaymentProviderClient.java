...
public class VnpayPaymentProviderClient implements PaymentProviderClient {
    ...
    public PaymentProviderResult createPayment(PaymentProviderRequest request) {
        ...
        String paymentUrl = properties.getPaymentUrl() + "?" + buildQuery(params, true) + "&vnp_SecureHash="
                + secureHash;
    ...
    public PaymentProviderCallbackResult parseCallback(Map<String, ?> payload) {
        ...
        BigDecimal amount = new BigDecimal(defaultString(payload.get("vnp_Amount"), "0"))
        ...
        return new PaymentProviderCallbackResult(txnRef, null, transactionId, amount,
                success ? PaymentStatus.SUCCESS : PaymentStatus.FAILED, eventKey, payload);
    }
    ...
    public PaymentProviderQueryResult queryPayment(PaymentAttempt attempt) {
        ...
        try {
            Map<String, Object> response = paymentRestClient.post().uri(properties.getTransactionUrl()).body(params)
            ...
            BigDecimal amount = new BigDecimal(defaultString(response.get("vnp_Amount"), "0"))
                    .divide(BigDecimal.valueOf(100));
            return new PaymentProviderQueryResult(attempt.getProviderTxnRef(), attempt.getRequestId(),
                    stringValue(response.get("vnp_TransactionNo")), amount, status,
                    stringValue(response.get("vnp_Message")), response.toString());
    ...
    private void requireEnabled() {
        if (!properties.isEnabled() || !StringUtils.hasText(properties.getPaymentUrl())
                || !StringUtils.hasText(properties.getTmnCode()) || !StringUtils.hasText(properties.getHashSecret())
