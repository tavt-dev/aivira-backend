...
public class ProductController {
    ...
    @Operation(summary = "Search public books", description = "Lists active bookstore products. Supports keyword, category, author, publisher, ISBN, price, availability, sorting, and pagination filters.")
    public ResponseEntity<ApiResponse<PageResponse<ProductResponse>>> getPublicProducts(
            @Parameter(description = "Keyword matched against title, SKU, description, brand, author, publisher, and ISBN", example = "clean code") @RequestParam(required = false) String keyword,
            @Parameter(description = "Book category slug", example = "programming") @RequestParam(required = false) String categorySlug,
            @Parameter(description = "Legacy brand filter kept for API compatibility", example = "Aivira") @RequestParam(required = false) String brand,
            @Parameter(description = "Case-insensitive author filter", example = "Robert C. Martin") @RequestParam(required = false) String author,
            @Parameter(description = "Case-insensitive publisher filter", example = "Prentice Hall") @RequestParam(required = false) String publisher,
            @Parameter(description = "ISBN contains filter", example = "9780132350884") @RequestParam(required = false) String isbn,
            @Parameter(description = "Minimum book price", example = "100000") @RequestParam(required = false) BigDecimal minPrice,
            @Parameter(description = "Maximum book price", example = "500000") @RequestParam(required = false) BigDecimal maxPrice,
            @Parameter(description = "When true, only books with available stock are returned", example = "true") @RequestParam(required = false) Boolean available,
            @Parameter(description = "Sort value: newest, price_asc, price_desc, best_selling, name_asc", example = "name_asc") @RequestParam(required = false) String sort,
            @Parameter(description = "1-based page number", example = "1") @RequestParam(defaultValue = "1") int page,
            @Parameter(description = "Page size", example = "20") @RequestParam(defaultValue = "20") int size) {
        return ResponseEntity.ok(
                ApiResponse.success("Get products successful", productService.getPublicProducts(keyword, categorySlug,
                        brand, author, publisher, isbn, minPrice, maxPrice, available, sort, page, size)));
    }
    ...
    @Operation(summary = "List products for admin", description = "Lists all books for catalog administration, including inactive and draft records when matched by filters.")
    ...
    public ResponseEntity<ApiResponse<PageResponse<ProductResponse>>> getAdminProducts(
            @RequestParam(required = false) ProductStatus status, @RequestParam(required = false) Long categoryId,
            @RequestParam(required = false) String keyword, @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "20") int size) {
        return ResponseEntity.ok(ApiResponse.success("Get admin products successful",
    ...
    public ResponseEntity<ApiResponse<ProductResponse>> getAdminProduct(@PathVariable Long productId) {
        return ResponseEntity
                .ok(ApiResponse.success("Get admin product successful", productService.getAdminProduct(productId)));
    }
    ...
    public ResponseEntity<ApiResponse<ProductResponse>> createAdminProduct(
        ...
        return ResponseEntity
                .ok(ApiResponse.success("Create product successful", productService.createAdminProduct(request)));
    }
    ...
    @PreAuthorize("@authorizationService.hasPermission('PRODUCT_MANAGE_ALL')")
    public ResponseEntity<ApiResponse<ProductResponse>> updateAdminProduct(@PathVariable Long productId,
            @Valid @RequestBody ProductUpdateRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Update product successful",
                productService.updateAdminProduct(productId, request)));
    }
    ...
    public ResponseEntity<ApiResponse<ProductResponse>> uploadProductMedia(@PathVariable Long productId,
        ...
        return ResponseEntity.ok(ApiResponse.success("Upload product media successful",
    ...
    @PreAuthorize("@authorizationService.hasAnyPermission('PRODUCT_MANAGE_ALL', 'PRODUCT_MEDIA_MANAGE_ALL')")
    public ResponseEntity<ApiResponse<ProductResponse>> updateProductMedia(@PathVariable Long productId,
            @PathVariable Long mediaId, @Valid @RequestBody ProductMediaUpdateRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Update product media successful",
                productService.updateProductMedia(productId, mediaId, request)));
    }
    ...
    @PreAuthorize("@authorizationService.hasAnyPermission('PRODUCT_MANAGE_ALL', 'PRODUCT_MEDIA_MANAGE_ALL')")
    public ResponseEntity<ApiResponse<Void>> deleteProductMedia(@PathVariable Long productId,
    ...
    @PreAuthorize("@authorizationService.hasPermission('PRODUCT_MANAGE_ALL')")
    public ResponseEntity<ApiResponse<ProductResponse>> createVariation(@PathVariable Long productId,
    ...
    @PreAuthorize("@authorizationService.hasPermission('PRODUCT_MANAGE_ALL')")
    public ResponseEntity<ApiResponse<ProductResponse>> updateVariation(@PathVariable Long productId,
            @PathVariable Long variationId, @Valid @RequestBody ProductVariationRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Update variation successful",
                productService.updateVariation(productId, variationId, request)));
    }
    ...
    @PreAuthorize("@authorizationService.hasPermission('PRODUCT_MANAGE_ALL')")
    public ResponseEntity<ApiResponse<Void>> deleteVariation(@PathVariable Long productId,
    ...
    public ResponseEntity<ApiResponse<ProductResponse>> updateVariationStock(@PathVariable Long productId,
            @PathVariable Long variationId, @Valid @RequestBody StockUpdateRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Update stock successful",
                productService.updateVariationStock(productId, variationId, request)));
    }
}
